...
parser.add_argument('--gamma2', type=float, default=300)
parser.add_argument('--lr_lbfgs', type=float, default=1e-4)
parser.add_argument('--lr_gd', type=float, default=1e-5)
parser.add_argument('--maxiter', type=int, default=1000)
parser.add_argument('--maxiter_bfgs', type=int, default=500)
parser.add_argument('--folds', type=int, default=10)
# gpu
parser.add_argument('--exp', type=str, default="main_A1_1_6")
...
def main():
    ...
    for fold in range(0, folds):
        ...
        if dataset == "emotions_new.mat":
            all = loadmat("datasets_kernel/kernel_{}".format(dataset))
            train_data = all["ten_fold_zscore_kernel_train_data"][fold][0]
            test_data  = all["ten_fold_zscore_kernel_test_data"][fold][0]

            train_data = [ train_data[i][0] for i in range(0, v_num)]
            test_data = [ test_data[i][0] for i in range(0, v_num)]
        else:
            import h5py
            all = h5py.File("datasets_kernel/kernel_{}".format(dataset), 'r')["all"]
            # print(all.keys())
            # exit()
            train_data, test_data = [], []
            for i in range(0, v_num):
                ten_fold_zscore_kernel_train_data_Cell = all["ten_fold_zscore_kernel_train_data"]
                ten_fold_zscore_kernel_train_data_Cell_Key = ten_fold_zscore_kernel_train_data_Cell[0][fold]
                obj = all[ten_fold_zscore_kernel_train_data_Cell_Key][0][i]
                obj_2 = all[obj][:]
                train_data.append(np.array(obj_2))

                ten_fold_zscore_kernel_test_data_Cell = all["ten_fold_zscore_kernel_test_data"]
                ten_fold_zscore_kernel_test_data_Cell_Key = ten_fold_zscore_kernel_test_data_Cell[0][fold]
                obj = all[ten_fold_zscore_kernel_test_data_Cell_Key][0][i]
                obj_2 = all[obj][:]
                test_data.append(np.array(obj_2).T)
        ...
        # bin_Y_test, bin_test_target = map(lambda x: binarization(torch.softmax(x, dim=1), 0.1), (bin_Y_test, bin_test_target))
        # HM = HammingLoss(bin_Y_test, bin_test_target)
    ...
    save_path = "results_kernel/exp={}/ds={}/r={}_p={}/".format(
...
def L_BFGS(parameters, non_parameters, loss_fun, maxiter=100, flag=""):
    optim = torch.optim.LBFGS([parameters], lr=args.lr_lbfgs, max_iter=maxiter, history_size=10000, line_search_fn="strong_wolfe")
    optim.zero_grad()
    def closure():
...
def GD(parameters, non_parameters, loss_fun, maxiter=1000, flag=""):
